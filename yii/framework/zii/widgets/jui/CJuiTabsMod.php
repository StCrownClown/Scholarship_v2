 ...
 * @since 1.1
 */
class CJuiTabsMod extends CJuiWidget
{
	/**
	...
	public function run()
	{
		...
		// if(is_null($this->options[selected]) && empty($this->options[selected])) $this->options[selected] = 0;
		
		$tmpOption = '';
		if(!empty($this->options)){
			foreach($this->options as $key => $value){
				$tmpOption .= CJavaScript::encode($key).':'.CJavaScript::encode($value).',';
			}
		}
		$options = '{
						collapsible:true,
						'.$tmpOption.'
						show: function( e,ui ){
							$( ui.panel ).html("<div class=\'tab-loading\'>Loading...</div>");
						}
					}';
