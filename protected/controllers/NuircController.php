...
class NuircController extends Controller {
    ...
    public function actionIndex() {
        if (Yii::app()->user->isGuest) {
            $model = new LoginForm ();
            Yii::app()->session['scholar_type'] = 'nuirc';
    ...
    public function actionGoto() {
        ...
        $nuircModel = new ScholarNuirc;
        if ($nuircModel->save()) {
                $scholarModel = new Scholar;
                $scholarModel['scholar_' . strtolower($scholar_type) . '_id'] = $nuircModel->id;
    ...
    public function actionExportExcelNuirc() {
        ...
        $sql = Yii::app()->params['sqlExportNuirc'] . " "
        ...
        if(!empty($data)){
            ...
            ExcelExporter::sendAsXLS('ExportExcelNuirc', $data, false, true, $fields);
...
//                'ind_type_service'=>'ประเภทการบริการ',
    ...
    public function actionExportAttachmentZip($id = NULL) {
        ...
        $ApplicationForm = Yii::app()->basePath . '/../template/nuirc/ApplicationForm/';
        ...
        if(!empty($Scholar)){
            $ZipName = 'NUIRC_'.$Scholar[0]['running'].'.zip';
            ...
            foreach ($Scholar as $nuirc) {
                $criteria = new CDbCriteria ();
                $criteria->condition = "id = " . $nuirc['x_id'];
                $criteria->limit = 1;
                $Sch = Scholar::model()->find($criteria);
                
                $zip->addDir($nuirc['running']);
                if (file_exists($ApplicationForm."ApplicationForm_".$nuirc['running'].".docx")) {
                    $newFile = $nuirc['running']."/"."ApplicationForm_".$nuirc['running'].".docx";
                    $zip->addFile($ApplicationForm."ApplicationForm_".$nuirc['running'].".docx", $newFile);
                }
                
                $zip->addDir($nuirc['running']."/"."Student"); // Add Folder
                $fd_pro_men = (empty($nuirc['pro_id'])?"Mentor":"Professor");
                $zip->addDir($nuirc['running']."/".$fd_pro_men); // Add Folder
                $zip->addDir($nuirc['running']."/"."Industrial"); // Add Folder
   
                if(is_dir($path.$nuirc['stu_id'])){
                    $pathStu = "/"."Student/";
                    $file = $Sch->student->image_path;
                    if (is_file($path.$file)) {
                        $newFile = $nuirc['running'].$pathStu.basename($file);
                        $zip->addFile($path.$file, $newFile);
                    }
                    $file = $Sch->scholarNuirc->student_transcript_path;
                    if (is_file($path.$file)) {
                        $newFile = $nuirc['running'].$pathStu.basename($file);
                        $zip->addFile($path.$file, $newFile);
                    }
                    $file = $Sch->scholarNuirc->student_portfolio_path;
                    if (is_file($path.$file)) {
                        $newFile = $nuirc['running'].$pathStu.basename($file);
                        $zip->addFile($path.$file, $newFile);
                    }        
                    $file = $Sch->scholarNuirc->student_attachment_other_path;
                    if (is_file($path.$file)) {
                        $newFile = $nuirc['running'].$pathStu.basename($file);
                        $zip->addFile($path.$file, $newFile);
                    } 
                    $file = $Sch->scholarNuirc->student_attachment_other2_path;
                    if (is_file($path.$file)) {
                        $newFile = $nuirc['running'].$pathStu.basename($file);
                        $zip->addFile($path.$file, $newFile);
                    }
                    $file = $Sch->scholarNuirc->student_attachment_other3_path;
                    if (is_file($path.$file)) {
                        $newFile = $nuirc['running'].$pathStu.basename($file);
                        $zip->addFile($path.$file, $newFile);
                    }
                ...
                if(is_dir($path.$nuirc['pro_id'])){
                    ...
                    $file = $Sch->scholarNuirc->professor_attachment_other_path;
                    if (is_file($path.$file)) {
                        $newFile = $nuirc['running'].$pathStu.basename($file);
                        $zip->addFile($path.$file, $newFile);
                    }
...
//                }
                ...
                if(is_dir($path.$nuirc['ind_id'])){
                    $pathInd = "/"."Industrial/";
                    $file = $Sch->industrial->image_path;
                    if (is_file($path.$file)) {
                        $newFile = $nuirc['running'].$pathInd.basename($file);
                        $zip->addFile($path.$file, $newFile);
                    }
                    $file = $Sch->scholarNuirc->industrial_certificate_path;
                    if (is_file($path.$file)) {
                        $newFile = $nuirc['running'].$pathInd.basename($file);
                        $zip->addFile($path.$file, $newFile);
                    }
                    $file = $Sch->scholarNuirc->industrial_attachment_other_path;
                    if (is_file($path.$file)) {
                        $newFile = $nuirc['running'].$pathInd.basename($file);
                        $zip->addFile($path.$file, $newFile);
                    }
    ...
    public function actionExportAllZip() {
        ...
        if(!empty($Scholar)){
            ...
            foreach ($Scholar as $nuirc) {
                ...
                $criteria->limit = 1;
                $Sch = Scholar::model()->find($criteria);
//                NuircController::actionWord($nuirc['x_id']);
                $zip->addDir($nuirc['running']);
                if (file_exists($ApplicationForm."ApplicationForm_".$nuirc['running'].".docx")) {
                    $newFile = $nuirc['running']."/"."ApplicationForm_".$nuirc['running'].".docx";
                    $zip->addFile($ApplicationForm."ApplicationForm_".$nuirc['running'].".docx", $newFile);
                ...
                if(is_dir($path.$nuirc['ind_id'])){
                    ...
                    $file = $Sch->scholarNuirc->industrial_join_path;
                    if (is_file($path.$file)) {
                        $newFile = $nuirc['running'].$pathInd.basename($file);
                        $zip->addFile($path.$file, $newFile);
                    }
...
//            $path = '/uploads/'.Yii::app()->session['person_id'].'/'.ConfigWeb::getActiveScholarId().'ApplicationForm.docx';
        ...
        if(!empty($data)){
            $document = $PHPWord->loadTemplate(Yii::app()->basePath . '/../template/nuirc/TemplateApplicationForm.docx');
...
//            $document->setValue('xxx', ConfigWeb::setCheck(NULL));
            ...
            $path = Yii::app()->basePath . '/../template/nuirc/ApplicationForm';
            ...
            if($download){
                $pathUrl = '/template/nuirc/ApplicationForm/ApplicationForm_'. $data[0]['running'] . '.docx';
                header('Location: ' . Yii::app()->baseUrl . $pathUrl);
                Yii::app()->end();
            }
        }else{
...
//                $this->redirect(Yii::app()->createUrl(WorkflowData::$home));
    ...
    public function actionRecommendation() {
        ...
        $scholar_nuirc_id = NULL;
        $model = new NuircRecommendationForm ();
        $sql = "SELECT 
                ...
                CONCAT(i.fname,'  ',i.lname) AS industrial_name,
                    ...
                    scholar_nuirc.project_name AS project_student_name,
                    scholar_nuirc.project_begin AS project_student_begin,
                    scholar_nuirc.project_end AS project_student_end,
                    scholar_nuirc.objective AS project_student_objective,
                    scholar_nuirc.expect AS project_student_expect,
                    scholar_nuirc.id AS scholar_nuirc_id
                FROM
                scholar
                LEFT JOIN scholar_nuirc ON scholar.scholar_nuirc_id = scholar_nuirc.id
                LEFT JOIN project ON scholar_nuirc.project_id = project.id
        ...
        foreach ($RecommendationDisplay [0] as $key => $value) {
            if ($key != 'scholar_nuirc_id')
                $model->$key = $value;
            else {
                $scholar_nuirc_id = $value;
                $criteria = new CDbCriteria ();
                $criteria->condition = "id = " . $scholar_nuirc_id;
                $criteria->limit = 1;
                $ScholarNuirc = ScholarNuirc::model()->find($criteria);
                // $model->industrial_support = $ScholarNuirc->industrial_support;
                $model->industrial_incash_salary = $ScholarNuirc->industrial_incash_salary;
                $model->industrial_incash_salary_cost = $ScholarNuirc->industrial_incash_salary_cost;
                $model->industrial_incash_rents = $ScholarNuirc->industrial_incash_rents;
                $model->industrial_incash_rents_cost = $ScholarNuirc->industrial_incash_rents_cost;
                $model->industrial_incash_traveling = $ScholarNuirc->industrial_incash_traveling;
                $model->industrial_incash_traveling_cost = $ScholarNuirc->industrial_incash_traveling_cost;
                $model->industrial_incash_other = $ScholarNuirc->industrial_incash_other;
                $model->industrial_incash_other_cost = $ScholarNuirc->industrial_incash_other_cost;
                $model->industrial_incash_other_text = $ScholarNuirc->industrial_incash_other_text;
                $model->industrial_incash_other2 = $ScholarNuirc->industrial_incash_other2;
                $model->industrial_incash_other2_cost = $ScholarNuirc->industrial_incash_other2_cost;
                $model->industrial_incash_other2_text = $ScholarNuirc->industrial_incash_other2_text;
                $model->industrial_inkind_equipment = $ScholarNuirc->industrial_inkind_equipment;
                $model->industrial_inkind_equipment_cost = $ScholarNuirc->industrial_inkind_equipment_cost;
                $model->industrial_inkind_other = $ScholarNuirc->industrial_inkind_other;
                $model->industrial_inkind_other_cost = $ScholarNuirc->industrial_inkind_other_cost;
                $model->industrial_inkind_other_text = $ScholarNuirc->industrial_inkind_other_text;
                $model->industrial_inkind_other2 = $ScholarNuirc->industrial_inkind_other2;
                $model->industrial_inkind_other2_cost = $ScholarNuirc->industrial_inkind_other2_cost;
                $model->industrial_inkind_other2_text = $ScholarNuirc->industrial_inkind_other2_text;
//                $model->industrial_support_desc = $ScholarNuirc->industrial_support_desc;
                ...
                $criteria->condition = " id = " . $ScholarNuirc->id;
                $criteria->limit = 1;
            }
        }

        if (Yii::app()->request->isPostRequest) {
            if (isset($_POST ['NuircRecommendationForm'])) {
                $model->attributes = Yii::app()->input->post('NuircRecommendationForm');
                ...
                if (isset($_POST ['draft']) || isset($_POST ['savesend']) || isset($_POST ['save'])) {
                    ...
                    if ($model->validate()) {
                        // $ScholarNuirc->industrial_support = $model->industrial_support;
                        // $ScholarNuirc->first_created = $ScholarNuirc->first_created;
                        // $ScholarNuirc->last_updated = date("Y-m-d H:i:s");
                        $ScholarNuirc->industrial_incash_salary = $model->industrial_incash_salary;
                        $ScholarNuirc->industrial_incash_salary_cost = $model->industrial_incash_salary_cost;
                        $ScholarNuirc->industrial_incash_rents = $model->industrial_incash_rents;
                        $ScholarNuirc->industrial_incash_rents_cost = $model->industrial_incash_rents_cost;
                        $ScholarNuirc->industrial_incash_traveling = $model->industrial_incash_traveling;
                        $ScholarNuirc->industrial_incash_traveling_cost = $model->industrial_incash_traveling_cost;
                        $ScholarNuirc->industrial_incash_other = $model->industrial_incash_other;
                        $ScholarNuirc->industrial_incash_other_cost = $model->industrial_incash_other_cost;
                        $ScholarNuirc->industrial_incash_other_text = $model->industrial_incash_other_text;
                        $ScholarNuirc->industrial_incash_other2 = $model->industrial_incash_other2;
                        $ScholarNuirc->industrial_incash_other2_cost = $model->industrial_incash_other2_cost;
                        $ScholarNuirc->industrial_incash_other2_text = $model->industrial_incash_other2_text;
                        $ScholarNuirc->industrial_inkind_equipment = $model->industrial_inkind_equipment;
                        $ScholarNuirc->industrial_inkind_equipment_cost = $model->industrial_inkind_equipment_cost;
                        $ScholarNuirc->industrial_inkind_other = $model->industrial_inkind_other;
                        $ScholarNuirc->industrial_inkind_other_cost = $model->industrial_inkind_other_cost;
                        $ScholarNuirc->industrial_inkind_other_text = $model->industrial_inkind_other_text;
                        $ScholarNuirc->industrial_inkind_other2 = $model->industrial_inkind_other2;
                        $ScholarNuirc->industrial_inkind_other2_cost = $model->industrial_inkind_other2_cost;
                        $ScholarNuirc->industrial_inkind_other2_text = $model->industrial_inkind_other2_text;
                        $ScholarNuirc->industrial_support_desc = $model->industrial_support_desc;
                        $ScholarNuirc->setIsNewRecord(FALSE);
                        if ($ScholarNuirc->update()) {
    ...
    public function actionProfessor() {
        ...
        if (Yii::app()->request->isPostRequest) {
            if (isset($_POST ['NuircStudentForm'])) {
                $model->attributes = Yii::app()->input->post('NuircStudentForm');
    ...
    public function actionVerifyStudent() {
        $ScholarNuirc = NULL;
        $model = new NuircStudentProjectForm ();
        $criteria = new CDbCriteria ();
        $criteria->condition = "id = " . ConfigWeb::getActiveScholarId();
        $criteria->limit = 1;
        $Scholar = Scholar::model()->find($criteria);
        if (!empty($Scholar->scholar_nuirc_id)) {
            $criteria->condition = "id = " . $Scholar->scholar_nuirc_id;
            $criteria->limit = 1;
            $ScholarNuirc = ScholarNuirc::model()->find($criteria);
            $model->attributes = $ScholarNuirc->attributes;
            $model->id = $ScholarNuirc->id;
        }
        
        if (Yii::app()->request->isPostRequest) {
            if (isset($_POST ['NuircStudentProjectForm'])) {
                $model->attributes = Yii::app()->input->post('NuircStudentProjectForm');
...
//                    $model->last_updated = date("Y-m-d H:i:s");
        ...
        if (Yii::app()->request->isPostRequest) {
            if (isset($_POST ['ScholarHistoryForm'])) {
                ...
                if ($model->validate()) {
                    $model->setIsNewRecord(FALSE);
                    if ($model->update()) {
                        $this->redirect(Yii::app()->createUrl('nuirc/history'));
                    }
                }
            }
        }
    ...
    public function actionHistory() {
        ...
        $model = new NuircHistoryForm ();
        $Nuirc = new ScholarNuirc ();
        $criteria = new CDbCriteria ();
        $criteria->condition = "id = $id";
        $criteria->limit = 1;
        $scholar = Scholar::model()->find($criteria);

        $criteria->condition = "id = " . $scholar->scholarNuirc->id;
        $criteria->limit = 1;
        $Nuirc = ScholarNuirc::model()->find($criteria);
        $model->attributes = $Nuirc->attributes;
        $model->id = $Nuirc->id;

        if (Yii::app()->request->isPostRequest) {
            if (isset($_POST ['NuircHistoryForm'])) {
                $model->attributes = Yii::app()->input->post('NuircHistoryForm');
                $Nuirc->is_history = $model->is_history;
                if ($model->validate()) {
                    $model->setIsNewRecord(FALSE);
                    if ($Nuirc->update()) {
                        $scholar->first_created = $scholar->first_created;
...
//		}
    ...
    public function actionWorking() {
        ...
        if (Yii::app()->request->isPostRequest) {
            if (isset($_POST ['NuircWorkingForm'])) {
                $model->attributes = Yii::app()->input->post('NuircWorkingForm');
                if ($model->validate()) {
                    $ScholarNuirc->is_work = $model->is_work;
                    $ScholarNuirc->work_company = $model->work_company;
                    $ScholarNuirc->work_position = $model->work_position;
                    $ScholarNuirc->work_location = $model->work_location;
                    $ScholarNuirc->work_phone = $model->work_phone;
                    $ScholarNuirc->work_fax = $model->work_fax;
                    $ScholarNuirc->is_workwithproject = $model->is_workwithproject;
                    $ScholarNuirc->workwithproject_text1 = $model->workwithproject_text1;
                    $ScholarNuirc->workwithproject_text2 = $model->workwithproject_text2;
                    $ScholarNuirc->workwithproject_text3 = $model->workwithproject_text3;
                    $ScholarNuirc->is_takescholar = $model->is_takescholar;
                    $model->setIsNewRecord(FALSE);
                    if ($ScholarNuirc->update()) {
                        $UrlNext = WorkflowData::WorkflowUrlNext(Yii::app()->urlManager->parseUrl(Yii::app()->request), TRUE);
                        $this->redirect(Yii::app()->createUrl($UrlNext));
    ...
    public function actionExperience() {
        ...
        if (Yii::app()->request->isPostRequest) {
            if (isset($_POST ['NuircExperienceForm'])) {
                $model->attributes = Yii::app()->input->post('NuircExperienceForm');
                if ($model->validate()) {
                    $ScholarNuirc->is_experience = $model->is_experience;
                    $ScholarNuirc->portfolio_thesis = $model->portfolio_thesis;
                    $ScholarNuirc->portfolio_journal_international = $model->portfolio_journal_international;
                    $ScholarNuirc->portfolio_journal_incountry = $model->portfolio_journal_incountry;
                    $ScholarNuirc->portfolio_patent = $model->portfolio_patent;
                    $ScholarNuirc->portfolio_prototype = $model->portfolio_prototype;
                    $ScholarNuirc->portfolio_conference_international = $model->portfolio_conference_international;
                    $ScholarNuirc->portfolio_conference_incountry = $model->portfolio_conference_incountry;
                    $ScholarNuirc->portfolio_award = $model->portfolio_award;
                    $ScholarNuirc->portfolio_other = $model->portfolio_other;
                    $ScholarNuirc->portfolio_journal_international_amount = $model->portfolio_journal_international_amount;
                    $ScholarNuirc->portfolio_journal_incountry_amount = $model->portfolio_journal_incountry_amount;
                    $ScholarNuirc->portfolio_patent_amount = $model->portfolio_patent_amount;
                    $ScholarNuirc->portfolio_prototype_amount = $model->portfolio_prototype_amount;
                    $ScholarNuirc->portfolio_conference_international_amount = $model->portfolio_conference_international_amount;
                    $ScholarNuirc->portfolio_conference_incountry_amount = $model->portfolio_conference_incountry_amount;
                    $ScholarNuirc->portfolio_award_amount = $model->portfolio_award_amount;
                    $ScholarNuirc->portfolio_other_text = $model->portfolio_other_text;
                    $ScholarNuirc->portfolio_other_amount = $model->portfolio_other_amount;
                    $ScholarNuirc->portfolio_journal_international_desc = $model->portfolio_journal_international_desc;
                    $ScholarNuirc->portfolio_journal_incountry_desc = $model->portfolio_journal_incountry_desc;
                    $ScholarNuirc->portfolio_patent_desc = $model->portfolio_patent_desc;
                    $ScholarNuirc->portfolio_prototype_desc = $model->portfolio_prototype_desc;
                    $ScholarNuirc->portfolio_conference_international_desc = $model->portfolio_conference_international_desc;
                    $ScholarNuirc->portfolio_conference_incountry_desc = $model->portfolio_conference_incountry_desc;
                    $ScholarNuirc->portfolio_award_desc = $model->portfolio_award_desc;
                    $ScholarNuirc->portfolio_other_desc = $model->portfolio_other_desc;
    ...
    public function actiongetData() {
        ...
        $model = new NuircPrimaryProjectForm('add');
        $criteria = new CDbCriteria ();
    ...
    public function actionPrimaryProject() {
        ...
        $person_id = ConfigWeb::getActivePersonId();
        $person_type = ConfigWeb::getActivePersonType();
        $model = new NuircPrimaryProjectForm($mode);
        $modelForm = new NuircPrimaryProjectForm ();
        $Project = NULL;
        $error = NULL;
        $Scholar = NULL;
        $criteria = new CDbCriteria ();
        ...
        if (!empty($Scholar->scholarNuirc->project_id)) {
            $criteria->condition = "id = " . $Scholar->scholarNuirc->project_id;
            $criteria->limit = 1;
            $Project = Project::model()->find($criteria);
            $model->attributes = $Project->attributes;
            $model->project_id = $Scholar->scholarNuirc->project_id;
        }
        ...
        if (Yii::app()->request->isPostRequest) {
            if (isset($_POST ['NuircPrimaryProjectForm'])) {
                $model->attributes = Yii::app()->input->post('NuircPrimaryProjectForm');
                $modelForm->attributes = Yii::app()->input->post('NuircPrimaryProjectForm');
                if (isset($_POST ['add'])) {
                    $model = new NuircPrimaryProjectForm('add');
                    $model->attributes = Yii::app()->input->post('NuircPrimaryProjectForm');
                    ...
                    if ($model->validate()) {
                        if ($model->save()) {
                            ...
                            $criteria->limit = 1;
                            $Scholar = Scholar::model()->find($criteria);
                            $Scholar->scholarNuirc->project_id = $model->id;
                            // $Scholar->scholarNuirc->last_updated = date("Y-m-d H:i:s");
                            $Scholar->scholarNuirc->setIsNewRecord(FALSE);
                            if ($Scholar->scholarNuirc->update()) {
                                $this->redirect(Yii::app()->createUrl('nuirc/primaryproject'));
                            }
                        }
                    }
                    ...
                    $model = new NuircPrimaryProjectForm('next');
                    $model->attributes = Yii::app()->input->post('NuircPrimaryProjectForm');
                    ...
                    if (!empty($model->project_id)) {
                        $Scholar->scholarNuirc->project_id = $model->project_id;
                        // $Scholar->scholarNuirc->last_updated = date("Y-m-d H:i:s");
                        $Scholar->scholarNuirc->setIsNewRecord(FALSE);
                        if ($Scholar->scholarNuirc->update()) {
        ...
        if (!empty($model->begin) && !empty($model->end)) {
            ...
            $begin_from = Yii::app()->params['nuirc_project_begin_min'];
            $begin_to = Yii::app()->params['nuirc_project_begin_max'];
            $end_last = Yii::app()->params['nuirc_project_sub_begin_min'];
...
//            $model->end = date('Y-m-d', strtotime($model->end . ' -1 day'));
    ...
    public function actionStudentProject() {
        ...
        if (!empty($ScholarNuirc->project_begin))
            $model->project_begin = date("d/m/Y", strtotime($ScholarNuirc->project_begin));
        if (!empty($ScholarNuirc->project_end))
            $model->project_end = date("d/m/Y", strtotime($ScholarNuirc->project_end));
    ...
    public function actionComment() {
        ...
        if (Yii::app()->request->isPostRequest) {
            if (isset($_POST ['NuircCommentForm'])) {
                $model->attributes = Yii::app()->input->post('NuircCommentForm');
    ...
    public function actionDownload($typedw = NULL, $scholar = NULL) {
        if ($typedw !== NULL) {
            ...
            if ($scholar == NULL) {
                ...
                $path = Yii::app()->basePath . Yii::app()->params['pathUploads'] . $Scholar->scholarNuirc[$typedw];
                if (file_exists($path)) {
                    return Yii::app()->getRequest()->sendFile($Scholar->scholarNuirc[$typedw], @file_get_contents($path));
                } else {
                    throw new CHttpException(404, 'The specified post cannot be found.');
                }
            }
...
//            $model->major_id = 0;
    ...
    public function actionMiniAttachment() {
        ...
        $model = new NuircUploadAttachmentForm();
        $criteria = new CDbCriteria;
        $criteria->condition = "id = $person_id";
        $criteria->limit = 1;
        $user = Person::model()->find($criteria);
        ...
        $model->attributes = $Scholar->scholarNuirc->attributes;
        $model->id = $Scholar->scholarNuirc->id;
        
        if($person_type == 'student'){
            $model->industrial_certificate_path = $Scholar->scholarNuirc->industrial_certificate_path;
            $model->student_transcript_path = $Scholar->scholarNuirc->student_transcript_path;
            $model->student_portfolio_path = $Scholar->scholarNuirc->student_portfolio_path;
            $model->student_attachment_other_path = $Scholar->scholarNuirc->student_attachment_other_path;
            $model->student_attachment_other2_path = $Scholar->scholarNuirc->student_attachment_other2_path;
            $model->student_attachment_other3_path = $Scholar->scholarNuirc->student_attachment_other3_path;
        }else if ($person_type == 'professor'){
            $model->cv_path = $user->cv_path;
            $model->professor_attachment_other_path = $Scholar->scholarNuirc->professor_attachment_other_path;
            $model->professor_attachment_other2_path = $Scholar->scholarNuirc->professor_attachment_other2_path;
            $model->professor_attachment_other3_path = $Scholar->scholarNuirc->professor_attachment_other3_path;
//                $model->professor_proposal_path = $Scholar->scholarNuirc->professor_proposal_path;
        }else if($person_type == 'mentor'){
            $model->cv_path = $user->cv_path;
            $model->mentor_attachment_other_path = $Scholar->scholarNuirc->mentor_attachment_other_path;
            $model->mentor_attachment_other2_path = $Scholar->scholarNuirc->mentor_attachment_other2_path;
            $model->mentor_attachment_other3_path = $Scholar->scholarNuirc->mentor_attachment_other3_path;
        }else if($person_type == 'industrial'){
            $model->cv_path = $user->cv_path;
            $model->industrial_attachment_other_path = $Scholar->scholarNuirc->industrial_attachment_other_path;
            $model->industrial_attachment_other2_path = $Scholar->scholarNuirc->industrial_attachment_other2_path;
            $model->industrial_attachment_other3_path = $Scholar->scholarNuirc->industrial_attachment_other3_path;
        }
        ...
        if (Yii::app()->request->isPostRequest) {
            if (isset($_POST['NuircUploadAttachmentForm'])) {
                $model->attributes = Yii::app()->input->post('NuircUploadAttachmentForm');
                if (isset($_POST['upload'])) {
                    if($person_type == 'student'){
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->student_transcript_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->student_portfolio_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->industrial_certificate_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->student_attachment_other_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->student_attachment_other2_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->student_attachment_other3_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->professor_attachment_other_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->professor_attachment_other2_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->professor_attachment_other3_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->mentor_attachment_other_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->mentor_attachment_other2_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->mentor_attachment_other3_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->industrial_attachment_other_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->industrial_attachment_other2_path = $fileName;
                        ...
                        if (!empty($FileUpload)) {
                            ...
                            $Scholar->scholarNuirc->id = $Scholar->scholarNuirc->id;
                            $Scholar->scholarNuirc->industrial_attachment_other3_path = $fileName;
...
//                        if (empty($Scholar->scholarNuirc->professor_attachment_other_path)){
//                            $error = 'error';
...
//                        }
                        if (empty($Scholar->scholarNuirc->student_transcript_path)) {
                            $error = 'error';
                            Yii::app()->user->setFlash('error2', $model->attributeLabels()['student_transcript_path'] . ' - กรุณาอัพโหลด / Please upload file');
                        }
                        if (empty($Scholar->scholarNuirc->industrial_certificate_path)){
                            $error = 'error';
