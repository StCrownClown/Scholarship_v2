                    ...
                    <div class="form-group login_content">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            ...
                            if (empty(Yii::app()->session['tmpReadOnly'])) {
                                ...
                                $addUrl = Yii::app()->createUrl('nuirc/primaryproject', array(
                                    'mode' => 'add'
                                ));
                            ...
                            $this->widget('booster.widgets.TbSelect2', array(
                                'asDropDownList' => TRUE,
                                'model' => $model,
                                'name' => 'NuircPrimaryProjectForm[project_id]',
...
?>
        $("#loader").fadeOut(100);
        $('input[type=radio][name="NuircPrimaryProjectForm[funding]"]').change(function () {
            $('#NuircPrimaryProjectForm_funding_name, #NuircPrimaryProjectForm_funding_code, #NuircPrimaryProjectForm_funding_code_name, #NuircPrimaryProjectForm_funding_etc').val('');
        });
    });

    function RadionButtonSelectedValueSet(name, SelectdValue) {
        $('input[name="NuircPrimaryProjectForm[funding]"][value="' + data.funding + '"]').prop('checked', true);
    }
    ...
    function showDetail(prj_id) {
        ...
        $("#loader").fadeIn(400);
        if (prj_id != '') {
            $.getJSON('<?php echo $this->createUrl('nuirc/getData'); ?>?prj_id=' + prj_id,
                    function (data) {
                        $('#NuircPrimaryProjectForm_id').val(data.id);
                        $('#NuircPrimaryProjectForm_name').val(data.name);
                        $('#NuircPrimaryProjectForm_objective').val(data.objective);
                        $('#NuircPrimaryProjectForm_scope').val(data.scope);
                        $('#NuircPrimaryProjectForm_begin').val(data.begin);
                        $('#NuircPrimaryProjectForm_end').val(data.end);
                        $('input[name="NuircPrimaryProjectForm[funding]"][value="' + data.funding + '"]').prop('checked', true);
                        $("#NuircPrimaryProjectForm_funding_name").hide();
                        $("#NuircPrimaryProjectForm_funding_code, #NuircPrimaryProjectForm_funding_code_name").hide();
                        $("#NuircPrimaryProjectForm_funding_etc").hide();
                        ...
                        $('#NuircPrimaryProjectForm_funding_name').val(data.funding_name);
                        $('#NuircPrimaryProjectForm_funding_code').val(data.funding_code);
                        $('#NuircPrimaryProjectForm_funding_code_name').val(data.funding_code_name);
                        $('#NuircPrimaryProjectForm_funding_etc').val(data.funding_etc);
                        $('#NuircPrimaryProjectForm_budget').val(data.budget);
                        $('#NuircPrimaryProjectForm_func_period').val(data.func_period);
