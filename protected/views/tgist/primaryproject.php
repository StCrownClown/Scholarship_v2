                    ...
                    <div class="form-group login_content">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            ...
                            if (empty(Yii::app()->session['tmpReadOnly'])) {
                                ...
                                $addUrl = Yii::app()->createUrl('tgist/primaryproject', array(
                                    'mode' => 'add'
                                ));
                            ...
                            $this->widget('booster.widgets.TbSelect2', array(
                                'asDropDownList' => TRUE,
                                'model' => $model,
                                'name' => 'TgistPrimaryProjectForm[project_id]',
...
?>
        $("#loader").fadeOut(100);
        $('input[type=radio][name="TgistPrimaryProjectForm[funding]"]').change(function () {
            $('#TgistPrimaryProjectForm_funding_name, #TgistPrimaryProjectForm_funding_code, #TgistPrimaryProjectForm_funding_code_name, #TgistPrimaryProjectForm_funding_etc').val('');
        });
    });

    function RadionButtonSelectedValueSet(name, SelectdValue) {
        $('input[name="TgistPrimaryProjectForm[funding]"][value="' + data.funding + '"]').prop('checked', true);
    }
    ...
    function showDetail(prj_id) {
        ...
        $("#loader").fadeIn(400);
        if (prj_id != '') {
            $.getJSON('<?php echo $this->createUrl('tgist/getData'); ?>?prj_id=' + prj_id,
                    function (data) {
                        $('#TgistPrimaryProjectForm_id').val(data.id);
                        $('#TgistPrimaryProjectForm_name').val(data.name);
                        $('#TgistPrimaryProjectForm_objective').val(data.objective);
                        $('#TgistPrimaryProjectForm_scope').val(data.scope);
                        $('#TgistPrimaryProjectForm_begin').val(data.begin);
                        $('#TgistPrimaryProjectForm_end').val(data.end);
                        $('input[name="TgistPrimaryProjectForm[funding]"][value="' + data.funding + '"]').prop('checked', true);
                        $("#TgistPrimaryProjectForm_funding_name").hide();
                        $("#TgistPrimaryProjectForm_funding_code, #TgistPrimaryProjectForm_funding_code_name").hide();
                        $("#TgistPrimaryProjectForm_funding_etc").hide();
                        if (data.funding == 'source') {
                            $("#TgistPrimaryProjectForm_funding_name").show();
                        } else if (data.funding == 'nstda') {
                            $("#TgistPrimaryProjectForm_funding_code, #TgistPrimaryProjectForm_funding_code_name").show();
                        } else if (data.funding == 'other') {
                            $("#TgistPrimaryProjectForm_funding_etc").show();
                        }
                        $('#TgistPrimaryProjectForm_funding_name').val(data.funding_name);
                        $('#TgistPrimaryProjectForm_funding_code').val(data.funding_code);
                        $('#TgistPrimaryProjectForm_funding_code_name').val(data.funding_code_name);
                        $('#TgistPrimaryProjectForm_funding_etc').val(data.funding_etc);
                        $('#TgistPrimaryProjectForm_budget').val(data.budget);
                        $('#TgistPrimaryProjectForm_func_period').val(data.func_period);
