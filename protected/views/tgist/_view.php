...
//    	$('#TgistPrimaryProjectForm_budget').keypress(function() {
//    		var nchar = String.fromCharCode(event.keyCode);
//    	    if ((nchar < '0' || nchar > '9') && (nchar != '.')) return false;
//    		Num = nchar;
//    		});
        
        $('#TgistPrimaryProjectForm_begin').datepicker({
            'autoclose': true,
            'orientation': "top",
            'format': 'dd/mm/yyyy',
        ...
        $('#TgistPrimaryProjectForm_end').datepicker({
            'autoclose': true,
            'orientation': "top",
            'format': 'dd/mm/yyyy',
            'viewformat': 'dd/mm/yyyy',
        ...
        $("input[name='TgistPrimaryProjectForm[funding]']").change(function () {
            $('div.error').hide();
            if($(this).val() == 'source'){
                $("label[for^='TgistPrimaryProjectForm_funding_name']").show();
                $('#TgistPrimaryProjectForm_funding_name').show();
                $("label[for^='TgistPrimaryProjectForm_funding_code']").hide();
                $('#TgistPrimaryProjectForm_funding_code, #TgistPrimaryProjectForm_funding_code_name').hide().val('');
                $("label[for^='TgistPrimaryProjectForm_funding_etc']").hide();
                $('#TgistPrimaryProjectForm_funding_etc').hide().val('');
            }else if($(this).val() == 'nstda'){
                $("label[for^='TgistPrimaryProjectForm_funding_name']").hide();
                $('#TgistPrimaryProjectForm_funding_name').hide().val('');
                $("label[for^='TgistPrimaryProjectForm_funding_code']").show();
                ...
                $("label[for^='TgistPrimaryProjectForm_funding_code']").hide();
                $('#TgistPrimaryProjectForm_funding_code, #TgistPrimaryProjectForm_funding_code_name').hide().val('');
                $("label[for^='TgistPrimaryProjectForm_funding_etc']").show();
    ...
    function setFuncPeriod() {
        var begin = $('#TgistPrimaryProjectForm_begin').val();
        var end = $('#TgistPrimaryProjectForm_end').val();
        $.ajax({
            url: '<?php echo Yii::app()->createUrl('tgist/get_func_period') ?>',
            data: {begin: begin, end: end, mode: 'primary'},
            type: "GET",
            success: function (data) {
                $('#TgistPrimaryProjectForm_func_period').val(data);
            },
            error: function (data) {
                $('#TgistPrimaryProjectForm_func_period').val(data);
            }
